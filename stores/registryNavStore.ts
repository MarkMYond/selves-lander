...
export const useRegistryNavStore = defineStore('registryNav', () => {
  ...
  const fetchNavItems = async (
  ...
  ): Promise<RegistryProcessedNavItem[]> => {
    if (parentId) {
      ...
      try {
        ...
        console.log(
          `[registryNavStore] Fetching children for page ID ${parentId} from navigation cache: ${url}`
        )

        const navItems = await $fetch<RegistryApiNavItem[]>(url)
        if (navItems && navItems.length > 0) {
          console.log(
            `[registryNavStore] Successfully fetched ${navItems.length} child items from MongoDB cache`
          )
          return processApiItemsRecursive(navItems)
        }

        console.warn(
          `[registryNavStore] No children found for parent ID ${parentId} in MongoDB cache, falling back to direct API`
        )
        ...
        console.log(
          `[registryNavStore] Falling back to direct API: ${fallbackUrl}`
        )
        ...
        console.error(
          `[registryNavStore] Error fetching children for page ${parentId}:`,
          error
        )
        return []
      }
    ...
    try {
      ...
      if (apiItems && apiItems.length > 0) {
        console.log(
          `[registryNavStore] Successfully fetched ${apiItems.length} items from MongoDB cache`
        )
        return processApiItemsRecursive(apiItems)
      }
      console.warn(
        '[registryNavStore] MongoDB cache returned empty navigation data'
      )
    } catch (cacheError) {
      console.error(
        '[registryNavStore] Failed to fetch from MongoDB cache:',
        cacheError
      )
    }
    ...
    try {
      console.log(
        `[registryNavStore] Falling back to static file: ${staticNavUrl}`
      )
      const staticItems = await $fetch<any[]>(staticNavUrl)
      return processApiItemsRecursive(staticItems)
    } catch (fallbackError) {
      console.error(
        '[registryNavStore] Static navigation file also failed:',
        fallbackError
      )
      return []
    }
  ...
  const findItemById = (
    items: RegistryProcessedNavItem[],
    id: string
  ): RegistryProcessedNavItem | null => {
  ...
  const toggleExpand = async (itemId: string) => {
    ...
    if (!item) {
      console.warn(
        `[registryNavStore] Item not found for toggleExpand: ${itemId}`
      ) // Changed log prefix
      return
    }
    ...
    if (
      ...
      try {
        ...
        console.error(
          `[registryNavStore] Failed to fetch children for ${item.title}:`,
          e
        ) // Changed log prefix
