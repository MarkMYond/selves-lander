...
export function usePayloadNavigation(sectionSlug: Ref<string | undefined>) { 
  ...
  const fetchNavData = async (section: string) => {
    ...
    try {
      ...
      error.value = err instanceof Error ? err : new Error('Unknown error fetching navigation');
  ...
  watch(sectionSlug, (newSection: string | undefined, oldSection: string | undefined) => {
