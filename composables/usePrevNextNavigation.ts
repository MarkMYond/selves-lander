 ...
 */
function flattenNavigation(navData: NavCategoryGroup[] | null): FlattenedNavPage[] {
  if (!navData || !Array.isArray(navData) || navData.length === 0) return [];

  const flattened: FlattenedNavPage[] = [];

  function recurse(pages: NavPage[], depth: number, categoryName: string, categoryId: string, parentId?: string) {
...
export function usePrevNextNavigation(
  ...
  const currentPageIndex = computed(() => {
    if (!currentPageSlug.value || !Array.isArray(flattenedNav.value) || flattenedNav.value.length === 0) return -1;
    return flattenedNav.value.findIndex(p => p.slug === currentPageSlug.value);
  });

  const currentPage = computed(() => {
    if (currentPageIndex.value === -1 || !Array.isArray(flattenedNav.value)) return null;
    return flattenedNav.value[currentPageIndex.value] || null;
  });

  const findOriginalPage = (pageId: string, categories: NavCategoryGroup[] | null): NavPage | null => {
  ...
  const getSiblingsAndIndex = (page: FlattenedNavPage, originalNavData: NavCategoryGroup[] | null): { siblings: NavPage[], currentIndex: number } | null => {
  ...
  const prevPage = computed((): FlattenedNavPage | null => {
    ...
    if (context && context.currentIndex > 0) {
      ...
      return flattenedNav.value.find(p => p.id === prevSiblingOriginal.id) || null;
    }
    return null;
  });
  ...
  const nextPage = computed((): FlattenedNavPage | null => {
    ...
    if (context && context.currentIndex < context.siblings.length - 1) {
      ...
      return flattenedNav.value.find(p => p.id === nextSiblingOriginal.id) || null;
    }
    return null;
  });
