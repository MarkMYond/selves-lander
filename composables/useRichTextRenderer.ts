  ...
  switch (node.type) {
    ...
    case 'heading': {
      ...
      if (tag === 'h2') {
        ...
        classes =
          'tracking-tight text-3xl leading-tight mb-4 max-md:text-2xl max-sm:text-xl'
      } else if (tag === 'h3') {
        ...
        // Color will be handled by prose styles
        classes =
          'text-2xl leading-8 max-sm:text-lg max-sm:leading-7 text-center mb-3'
      }
...
export function useRichTextRenderer(
  richTextRoot: Readonly<Ref<RichTextRoot | undefined | null>>
) {
  // Reverted parameter type
