...
import type { WebPage, WikiPage, RegistryPage } from '../../payload-cms/src/payload-types'; // Import other page types
 ...
 */
export function useSeo(page: SeoPageData | undefined, type: 'website' | 'article' = 'website') {
  if (!page) {
    return;
  }
  ...
  const imageUrl = (typeof meta?.image === 'object' && meta.image?.url) ? meta.image.url : '';
  const ogType = type;
