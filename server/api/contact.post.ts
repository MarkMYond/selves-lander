...
export default defineEventHandler(async (event) => {
  ...
  const resendApiKey = runtimeConfig.resendApiKey || process.env.NUXT_RESEND_API_KEY; // Prefer runtimeConfig if available
  ...
  try {
    ...
    const { 
      ...
      subject, 
      message, 
      recipientEmail // Now validated to be a non-empty string
    } = body as { // Type assertion for clarity after validation
      name?: string;
      email: string;
      company?: string;
      subject?: string;
    ...
    const configuredFromEmail = runtimeConfig.public.resendFromEmail || process.env.NUXT_PUBLIC_RESEND_FROM_EMAIL;
    if (!configuredFromEmail || typeof configuredFromEmail !== 'string' || configuredFromEmail.trim() === '') {
      ...
      // Using a hardcoded default for 'from' is often problematic for deliverability.
      // It's better to throw an error if not configured.
      throw createError({
        statusCode: 500,
    ...
    // Ensure recipientEmail is definitely a string for the Resend SDK
    ...
    const emailToSend = {
      ...
      text: emailTextContent, // Added plain text version
    };
    ...
    if (error.statusCode && error.statusMessage) {
      throw error; // Re-throw if it's already a createError instance
    }
    throw createError({
      statusCode: 500,
