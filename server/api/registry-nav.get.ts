...
const buildPayloadQueryUrl = (
  ...
  for (const key in params) {
    if (key === 'where') { // Special handling for 'where' clauses
...
export default defineEventHandler(async (event): Promise<NavItem[]> => {
  ...
  console.log(`[/api/registry-nav] TRACE: runtimeConfig.public.payloadApiUrl: ${payloadApiUrl}`); // Changed log prefix
  
  if (!payloadApiUrl) {
    console.error('[/api/registry-nav] FATAL: payloadApiUrl is not defined in runtime config.'); // Changed log prefix
  ...
  const requestUrl = buildPayloadQueryUrl(payloadApiUrl, 'registry-pages', { // Changed collection to 'registry-pages'
    where: whereClauses,
    sort: 'sort',
    limit: 100, 
  ...
  console.log(`[/api/registry-nav] TRACE: Attempting to fetch parent/sibling items from: ${requestUrl}`); // Changed log prefix

  try {
    const response = await $fetch<{ docs: PayloadDoc[], totalDocs: number }>(requestUrl, {
      method: 'GET',
      headers: { 'Content-Type': 'application/json' },
    });

    if (!response || !response.docs) {
      console.warn('[/api/registry-nav] WARN: No docs in response or invalid response from Payload API for parent/siblings.'); // Changed log prefix
      return []; // Return empty array if no items found
    }

    console.log(`[/api/registry-nav] TRACE: Received ${response.docs.length} parent/sibling docs from Payload API.`); // Changed log prefix

    const navItemsPromises = response.docs.map(async (doc: PayloadDoc) => {
      // For each doc, check if it has children
      const childrenCheckUrl = buildPayloadQueryUrl(payloadApiUrl, 'registry-pages', { // Changed collection to 'registry-pages'
      ...
      console.log(`[/api/registry-nav] TRACE: Checking children for doc ID ${doc.id} from: ${childrenCheckUrl}`); // Changed log prefix
      const childrenResponse = await $fetch<{ docs: PayloadDoc[], totalDocs: number }>(childrenCheckUrl);
      const hasChildren = childrenResponse && childrenResponse.totalDocs > 0;
      console.log(`[/api/registry-nav] TRACE: Doc ID ${doc.id} hasChildren: ${hasChildren}`); // Changed log prefix
    ...
    console.log('[/api/registry-nav] TRACE: Processed navItems with hasChildren:', navItems.length); // Changed log prefix
    return navItems;

  } catch (err: any) {
    const fetchError = err as FetchError;
    console.error(`[/api/registry-nav] ERROR: Error during registry navigation data processing. Initial URL: ${requestUrl}`, fetchError.message); // Changed log prefix and message
    if (fetchError.response) {
      console.error('[/api/registry-nav] ERROR: Payload API Response Status:', fetchError.response.status); // Changed log prefix
      console.error('[/api/registry-nav] ERROR: Payload API Response Data:', fetchError.response._data); // Changed log prefix
    }
