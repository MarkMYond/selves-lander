  ...
  <main>
    <section class="book-a-demo-form-section py-16 sm:py-24 lg:py-32">
      <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        <div
          class="book-a-demo-card max-w-2xl mx-auto bg-white p-8 md:p-12 rounded-xl shadow-xl"
        >
          <h3 class="text-2xl font-bold text-brandNeutral-04 mb-8 text-center">
            Book Your Demo
          </h3>
          <form
            class="space-y-6"
          ...
          >
            <div>
              <label
                for="your-name"
                class="block text-sm font-medium text-brandNeutral-03 mb-1"
              >Your Name</label>
              <input
                id="your-name"
                type="text"
                ...
                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-brandTheme-01 focus:border-brandTheme-01 sm:text-sm"
              >
            </div>
            <div>
              <label
...
const { data: pageData, error } = await useAsyncData<PageType>(
  ...
  () =>
    $fetch(
      `${PAYLOAD_API_URL}/web-pages?where[slug][equals]=book-a-demo&depth=2`
    ).then((data) => (data as any)?.docs?.[0] as PageType)
)
