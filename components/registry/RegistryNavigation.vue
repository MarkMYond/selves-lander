<template>
  ...
  >
    ...
    <!-- Loop through processed items: could be categories or top-level pages -->
          ...
          <ul v-if="item.expanded && item.children && item.children.length > 0" :id="`category-children-${item.id}`" class="ml-4 mt-1 pl-3 border-l border-primary-200 space-y-1">
            ...
            <li v-for="page in item.children" :key="page.id" class="page-item">
              <div class="flex items-center justify-between group">
                ...
                >
                  <!-- Icons commented out to match WikiNavigation current state -->
                  <!--
                  <component
                    v-if="page.icon"
    ...
    <p v-else-if="!navStore.pending && (!navStore.processedNavigationItems || navStore.processedNavigationItems.length === 0)">
      No categories or pages found for the registry. <!-- Updated text -->
    </p>
    <p v-if="navStore.pending && !navStore.isInitialized" class="text-gray-600">
      Loading navigation...
    </p>
    <p v-if="navStore.error" class="text-red-500">
