  ...
  <section :class="[mappedSectionBgColor, ...sectionPaddingClasses]">
    ...
    >
      <SectionHeader
        ...
        :title="title"
        :subtitle="
          block.mainCtaText && block.mainCtaLink
            ? null
        ...
        :theme="
          sectionBgColorValue === 'brand-900' ||
            sectionBgColorValue === 'brand-primary'
            ? 'dark'
            : 'light'
        "
      />
      ...
      <div class="flex justify-center items-center space-x-3 mb-8 md:mb-12">
        <span
          :class="{
            'font-semibold text-brand-primary': !isYearlyBilling,
            'text-gray-500': isYearlyBilling,
          }"
        >Monthly</span>
        <button
          type="button"
      ...
      <div
        v-if="plans.length <= 1 && mainCtaLink"
        class="text-center mt-8 mb-0"
      >
        <NuxtLink
      ...
      >
        <PlanCard
          ...
          :class="
            plans.length === 1
              ? 'md:col-span-2 lg:col-span-3 md:max-w-md lg:max-w-md mx-auto'
              : plans.length === 2 && index === 0
      ...
      <div
        v-if="
          block.sharedFeatures &&
            block.sharedFeatures.length > 0 &&
            block.compareTableTitle
        "
      ...
      >
        ...
        />
        <div
          class="overflow-x-auto bg-white dark:bg-brandNeutral-03 shadow-md rounded-lg"
        >
          <table
          ...
          >
            ...
            <tbody
              class="bg-white dark:bg-brandNeutral-03 divide-y divide-gray-200 dark:divide-gray-700"
            >
              <tr
                v-for="(feature, featureIndex) in block.sharedFeatures"
                :key="`feature-row-${feature.id || featureIndex}`"
              >
                <td
                ...
                <td
                  ...
                  :key="`cell-${feature.id}-${plan.id || planIndex}`"
                  class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300 text-center"
                >
                  <template
                    v-if="
                      getFeatureAvailability(feature, planIndex) === 'included'
                    "
                  >
                    <Icon
                      name="ph:check-circle-fill"
                      class="text-green-500 h-5 w-5 mx-auto"
                    />
                  </template>
                  <template
                    v-else-if="
                      getFeatureAvailability(feature, planIndex) ===
                        'not_included'
                    "
                  >
                    <Icon
                      name="ph:x-circle-fill"
                      class="text-red-400 h-5 w-5 mx-auto"
                    />
                  </template>
                  <template v-else>
...
const title = computed(
  () => props.block?.title || 'Plans for every stage of your company'
)
...
const mappedSectionBgColor = computed(() => {
  switch (sectionBgColorValue.value) {
    case 'light-grey':
      return 'bg-brandNeutral-02 dark:bg-brandNeutral-04'
    case 'brand-50':
...
const sectionPaddingClasses = computed(() => {
  ...
  if (props.block?.removeTopPadding) {
    ...
    classes.push('py-12 md:py-16 lg:py-20')
  }
...
const getMainButtonClasses = () => {
  const baseClasses =
    'inline-flex items-center justify-center px-8 py-3 text-base font-medium rounded-md shadow-sm transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2'
...
const getCardBgClass = (colorClass?: string | null) => {
  return colorClass && colorClass.startsWith('bg-')
    ? colorClass
    : 'bg-white dark:bg-brandNeutral-03'
}
