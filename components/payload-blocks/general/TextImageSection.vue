...
const props = defineProps<{
  block: Omit<TextImageSectionBlock, 'containerWidth'> & { // Omit containerWidth as it's removed from Payload
    id: string
...
const imageBgClass = computed(() => {
  if (props.block?.imageBackgroundColor && props.block.imageBackgroundColor !== 'none') {
...
const dynamicPaddingClass = computed(() => {
  ...
  if (topPadding === 'pt-16' && bottomPadding === 'pb-16') return 'block-section-padding'; // py-16
...
<template>
  <section :class="[sectionBgClass, dynamicPaddingClass, 'text-image-feature-item']"> <!-- Use dynamic padding class and add static hook -->
    <!-- Removed the empty div: <div class="mt-16 bg-zinc-50 max-md:mt-14 max-sm:mt-9"></div> -->
    <div class="relative" :class="[contentBgClass]"> <!-- Removed lg:px-40 -->
      ...
      >
        ...
        <div class="flex-1">
          <div
            class="flex-1 mx-auto max-w-[618px] max-md:max-w-none" 
          ><!-- Removed px-4 lg:px-0 -->
            ...
            <!-- Changed back to <p> with direct styles -->
            <!-- Buttons -->
            <div
        ...
        <!-- Image Area -->
        <div v-if="imageUrl" class="flex-1 rounded-xl flex items-end justify-center px-4 pt-[42px]" :class="[imageBgClass]">
          <img
            :src="imageUrl"
            :alt="imageAlt"
