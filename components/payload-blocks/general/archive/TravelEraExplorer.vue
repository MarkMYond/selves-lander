  ...
  <section class="travel-era-explorer py-16 md:py-24 bg-slate-50">
    <div class="container mx-auto px-4">
      ...
      <div class="text-center mb-16">
        <h2 class="text-3xl md:text-4xl font-bold mb-4">{{ heading || "Travel Through Technology Eras" }}</h2>
      ...
      <div class="travel-section mb-16">
        <div class="travel-header text-center mb-8">
          <h3 class="text-2xl font-bold mb-2">{{ travelHeading || "Travel through time" }}</h3>
          <p class="text-gray-600 max-w-2xl mx-auto">{{ travelDescription }}</p>
        </div>
        
        <!-- Era Categories/Filters (optional) -->
        <div v-if="categories && categories.length" class="filter-categories flex flex-wrap justify-center gap-2 mb-8">
          <button 
            ...
            @click="setActiveCategory(category.id)"
              :class="[
                ...
                activeTab === tab.id
                  ...
                  : 'bg-white border border-brand-primary text-gray-700 hover:bg-gray-50' // Changed border-gray-200 to border-brand-primary
            ]"
            >
            {{ category.name }}
          </button>
        ...
        <div class="timeline-nav relative flex justify-between items-center mb-12 max-w-4xl mx-auto">
          <div class="timeline-track h-1 bg-gray-300 absolute w-full"></div>
          <div 
            ...
            <div 
              ...
              :class="[
                activeEraId === era.id 
                  ...
                  : 'border-brand-primary bg-white hover:border-brand-primary' // Changed border-gray-400 and hover:border-primary-400
              ]"
            ></div>
            <div 
              ...
              :class="{ 'text-primary-600': activeEraId === era.id, 'text-gray-600': activeEraId !== era.id }"
            >
              {{ era.name }}
            </div>
          </div>
        ...
        <!-- Active Era Content -->
        <div v-if="activeEra" class="era-content mt-16">
          <div class="flex flex-col md:flex-row gap-8 items-start">
            ...
            <div class="era-image md:w-1/2">
              ...
              />
              <div v-else class="bg-gray-100 rounded-lg w-full aspect-video flex items-center justify-center">
                <span class="text-gray-400 [font-variant-ligatures:stylistic] [font-feature-settings:\'ss01\']">No image available</span>
              </div>
            </div>
            
            <!-- Era Details -->
            <div class="era-details md:w-1/2">
              <h3 class="text-2xl font-bold mb-4">{{ activeEra.title }}</h3>
              <p class="text-gray-700 mb-6 [font-variant-ligatures:stylistic] [font-feature-settings:\'ss01\']">{{ activeEra.description }}</p>
              
              <!-- Key Features List -->
              <div v-if="activeEra.features && activeEra.features.length" class="key-features mb-6">
                <h4 class="font-semibold text-lg mb-2">Key Features</h4>
                <ul class="list-disc pl-5 space-y-1">
                  <li v-for="(feature, index) in activeEra.features" :key="index" class="text-gray-700 [font-variant-ligatures:stylistic] [font-feature-settings:\'ss01\']">
                    {{ feature }}
                  </li>
                </ul>
              </div>
              ...
              <!-- Technologies List -->
              <div v-if="activeEra.technologies && activeEra.technologies.length" class="technologies">
...
const filteredEras = computed(() => {
  ...
  return props.travelEras.filter(era => {
    // Check if era has the active category
...
const setActiveCategory = (categoryId) => {
  ...
  // Reset active era when changing categories
  activeEraId.value = '';
};
