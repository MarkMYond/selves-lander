<template>
  <section 
    ...
    :data-w-id="props.block?.animationId || '504b0f0e-891f-785a-1291-8c4be535afcc'"
    ref="sectionRef"
    style="opacity:0" 
  >
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <div class="max-w-4xl mx-auto">
        <div class="hero-blog-card flex flex-col md:flex-row md:justify-between p-6 md:p-8 bg-brandNeutral-02 rounded-xl shadow-lg">
          <div class="blog-left-block md:w-2/5 flex flex-col justify-between space-y-6 mb-8 md:mb-0">
            <div class="hero-blog-category-content space-y-4">
              <div v-if="props.block?.categoryText" class="blog-category inline-block">
                <p class="text-body-14 font-medium bg-brandTheme-02 text-brandNeutral-04 px-3 py-1 rounded-full">
              ...
              <div class="hero-blog-content space-y-3">
                ...
                >
                  <h2 class="blog-card-title-large text-h3 md:text-h2 font-bold text-brandNeutral-04 group-hover:text-brandTheme-01 transition-colors leading-[1.1]">
                    {{ props.block.title }}
                  </h2>
                </NuxtLink>
                <h2 v-else-if="props.block?.title" class="blog-card-title-large text-h3 md:text-h2 font-bold text-brandNeutral-04 leading-[1.1]">
                    {{ props.block.title }}
                </h2>
                <p v-if="props.block?.excerpt" class="blog-excerpt text-brandNeutral-03 text-body-16 lg:text-body-18">
                  {{ props.block.excerpt }}
                </p>
              </div>
            </div>
          ...
          <NuxtLink 
            v-if="props.block?.image && typeof props.block.image === 'object' && props.block.image.url && isLinkValid"
            :to="getLinkUrl(props.block.link)" 
            :target="props.block.link?.newTab ? '_blank' : undefined"
          ...
          >
            <img 
              ...
              loading="lazy" 
              :alt="props.block.image.alt || props.block.title || 'Blog post image'" 
...
const getLinkUrl = (linkData?: LinkData | null): string => {
  ...
  if (linkData.type === 'internal' && linkData.internalLink) {
    ...
    if (typeof internalValue === 'object' && internalValue !== null && 'slug' in internalValue && typeof (internalValue as WebPage).slug === 'string') {
...
<style scoped>
/* Scoped styles specific to HeroBlogCardSection */
</style>
