...
/**
 * Script to generate a static wiki navigation JSON file
...
const OUTPUT_FILE = path.join(OUTPUT_DIR, 'wiki-navigation.json');
...
async function getPageChildren(pageId, payloadApiUrl) {
  ...
  const childNavItems = await Promise.all(childrenData.docs.map(async (childDoc) => {
    const grandChildrenUrl = buildPayloadQueryUrl(payloadApiUrl, 'wiki-pages', {
      where: { 
...
async function generateNavigation() {
  console.log('Starting wiki navigation generation...');
  ...
  try {
    ...
    console.error('Error generating wiki navigation:', err);
    throw err;
  }
}
...
if (require.main === module) {
  generateNavigation()
    .then(() => {
      console.log('Wiki navigation generation completed successfully.');
      process.exit(0);
    })
    .catch((err) => {
      console.error('Wiki navigation generation failed:', err);
      process.exit(1);
    });
}
